...
TEUCHOS_UNIT_TEST(Trapezoidal, Default_Construction)
{
  ...
  auto observer  = rcp(new Tempus::StepperTrapezoidalObserverDefault<double>());
  auto solver    = rcp(new Thyra::NOXNonlinearSolver());
  solver->setParameterList(Tempus::defaultSolverParameters());

  auto defaultStepper = rcp(new Tempus::StepperTrapezoidal<double>());
...
{
  auto model = rcp(new Tempus_Test::SinCosModel<double>());
  testFactoryConstruction("Trapezoidal", model);
}
...
TEUCHOS_UNIT_TEST(Trapezoidal, AppAction_Observer)
{
  ...
  TEST_COMPARE(observer->testType, ==, "Trapezoidal");
}
