...
void MeshDiagnosticObserver::finished_modification_end_notification()
{
    if(m_diagnosticFlag)
    {
        ...
        {
            EntityVector orphanedSides = stk::mesh::get_orphaned_sides_with_attached_element_on_different_proc(m_bulkData);
        ...
        {
            EntityVector orphanedSides = stk::mesh::get_solo_sides_without_element_on_different_proc(m_bulkData);
